...
def zs_handle_detection(case_file: CaseFile, TEST=False) -> CaseFile:
    ...
    for detection in case_file.detections:
        ...
        ticket_number = case_file.get_ticket_number()
        device_name = detection.device.name if hasattr(detection, "device") else "unknown_device"
        process_name = detection.process.process_name if hasattr(detection, "process") else "unknown_process"
        process_id = detection.process.process_uuid if hasattr(detection, "process") else "unknown_process id"
        destination_ip = detection.destination_ip if hasattr(detection, "destination_ip") else "destination_ip"

        command = [
        "python",  # Python interpreter
        "playbooks/file_flow.py",  # The external script to call
        str(ticket_number),  # Pass the ticket number
        device_name,  # Pass the device name
        process_name  # Pass the process name
         ]
        command2 = [
        "python",  # Python interpreter
        "playbooks/registry_flow.py",  # The external script to call
        str(ticket_number),  # Pass the ticket number
        device_name,  # Pass the device name
        process_name  # Pass the process name
         ]
        command3 = [
        "python",  # Python interpreter
        "playbooks/process_tree.py",  # The external script to call
        str(ticket_number),  # Pass the ticket number
        device_name,  # Pass the device name
        process_id  # Pass the process id
         ]
        command4 = [
        "python",  # Python interpreter
        "playbooks/network_flow.py",  # The external script to call
        str(ticket_number),  # Pass the ticket number
        device_name,  # Pass the device name
        process_name  # Pass the process name
         ]
        ...
        command5 = [
        "python",  # Python interpreter
        "playbooks/DNS_flow.py",  # The external script to call
        str(ticket_number),  # Pass the ticket number
        device_name,  # Pass the device name
        process_name  # Pass the process name
         ]
        command6 = [
        "python",  # Python interpreter
        "playbooks/URL_flow.py",  # The external script to call
        str(ticket_number),  # Pass the ticket number
        device_name,  # Pass the device name
        destination_ip  # Pass the process name
         ]
